...
def precipitation_type_calculator(filename,
    ...
    if not isinstance(modelMetadataDictionary, dict):
        raise TypeError(
            "modelMetdataDictionary must be a dictionary containing the metadata about the snow level, temperature, and ground temperature files")
...
def generate_interpolations(model_reprojected_data, nwc_timestamps, startdate, timeStep=5, timeBase=60, dateFormat='%Y%m%d%H%M'):
    ...
    resultMatrix = np.zeros((model_start + len(timestamp_selection), model_reprojected_data.shape[1], model_reprojected_data.shape[2]))
    result_idx = 0
    ...
    for m in range(1, model_reprojected_data.shape[0]):
        if result_idx < resultMatrix.shape[0]:
            ...
            interpolationMatrix = grid_interpolation(model_reprojected_data[m-1], model_reprojected_data[m], timeStep=timeStep, timeBase=timeBase)
            interp_idx = 0
